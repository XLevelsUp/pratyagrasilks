...
export default function AddressCard({
    ...
    const getLabelIcon = () => {
        switch (label) {
            case 'Home': return <Home className="w-3.5 h-3.5 mr-1" />;
            case 'Work': return <Briefcase className="w-3.5 h-3.5 mr-1" />;
            default: return <MapPin className="w-3.5 h-3.5 mr-1" />;
        }
    };

    const getLabelStyles = () => {
        switch (label) {
            case 'Home': return 'bg-blue-50 text-blue-700 border-blue-100';
        ...
        >
            ...
            <div className={`p-5 flex-grow ${is_default ? 'pt-9' : ''}`}>
                <div className="flex items-center justify-between mb-3.5">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getLabelStyles()}`}>
                        {getLabelIcon()}
                        {label}
                    </span>
                    {is_default && (
            ...
            <div className="bg-gray-50 px-5 py-3 border-t border-gray-100 flex items-center justify-between gap-2.5">
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="p-1.5 text-gray-500 hover:text-accent hover:bg-accent-light rounded-lg transition-all duration-200 disabled:opacity-50"
                        title="Edit Address"
                    >
                ...
                {!is_default ? (
                    <button
                        ...
                        disabled={isProcessing}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-300 hover:border-accent text-xs font-semibold text-gray-700 hover:text-accent hover:bg-accent-light rounded-lg transition-all duration-200 shadow-sm disabled:opacity-50"
                    >
                        Set as Default
                    </button>
                ) : (
