...
export default function AddressModal({
    ...
    const fieldClass = (err?: string) =>
        `w-full px-3.5 py-2 border rounded-lg text-sm transition-all focus:outline-none focus:ring-2 ${
            err ? 'border-red-400 focus:ring-red-100' : 'border-gray-300 focus:border-accent focus:ring-accent-light'
        }`;
            ...
            <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden z-10 border border-gray-100">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-grow overflow-y-auto">
                    <div className="p-6 space-y-5">
                        ...
                        {/* Label pills */}
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2.5">Address Type / Label</label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="First and Last name" className={fieldClass(errors.fullName)} />
                        ...
                        {/* Address Line 1 */}
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-1.5">Flat, House no., Building <span className="text-red-500">*</span></label>
                            <input type="text" value={addressLine1} onChange={(e) => setAddressLine1(e.target.value)} placeholder="Street address, P.O. box, company name" className={fieldClass(errors.addressLine1)} />
                            {errors.addressLine1 && <p className="mt-1 text-xs font-medium text-red-500">{errors.addressLine1}</p>}
                        </div>

                        {/* Address Line 2 */}
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-1.5">Area, Street, Sector <span className="text-gray-400 text-xs font-normal">(Optional)</span></label>
                            <input type="text" value={addressLine2} onChange={(e) => setAddressLine2(e.target.value)} placeholder="Apartment, suite, unit, building, floor" className="w-full px-3.5 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent-light" />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                {errors.postalCode && <p className="mt-1 text-xs font-medium text-red-500">{errors.postalCode}</p>}
                            </div>
                        </div>
                        ...
                        {!address?.is_default && (
                            <div className="flex items-center gap-2 pt-2">
                                <input type="checkbox" id="isDefaultCheckbox" checked={isDefault} onChange={(e) => setIsDefault(e.target.checked)} className="w-4 h-4 text-accent border-gray-300 rounded focus:ring-accent-light focus:ring-2 cursor-pointer" />
                                <label htmlFor="isDefaultCheckbox" className="text-sm font-medium text-gray-700 cursor-pointer select-none">
